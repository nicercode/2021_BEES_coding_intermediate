...
author: Daniel Falster & Will Cornwell
...
for(f in unique(data$family)) {
  ...
  n[[f]] <- data.frame(family=f, n = length(unique(data_sub$scientificName)))
}
bind_rows(n)
```
...
**Exercise**: Using the pipe, write expressions to
...
```{r}
data %>% arrange(scientificName) %>% distinct(scientificName) 
```

3. List all the species names in reverse alphabetic order

```{r}
data$scientificName %>% unique() %>% sort() %>% rev()
...
```{r}
data %>% select(scientificName, genus, ends_with("tude")) %>% 
  filter(genus == "Aonyx") %>% 
  summarise(total_n = length(unique(.)))
```
...
data_subset <- 
  ...
  summarise(nspp = n_distinct(Species), max_lat = max(Latitude))
```
...
**Exercise:** Use dplyr's group_by function to get for each species, the number of records, min and max of decimalLatitude
...
data$scientificName %>% unique() %>% str_length()
```
...
Alternative way
```{r}
data <- list.files("data/species", full.names = TRUE) %>% map(read_csv) %>% bind_rows()
```
...
list.files("data/species", full.names = TRUE) %>% map(read_csv) %>% bind_rows()
```

A simpler example, print letters 1:10

```{r}
tmp <- map(letters[1:10], cat)
```
...
Try something similar with our data
...
Let's copy files to another directory

```{r}
input <- list.files("data/species", full.names = TRUE)
output <- list.files("data/species") %>% paste0("data - ", .) %>% file.path("output",.)

map2(input, output, file.copy)
```


Could also use `walk2`

```{r}
walk2(input, output, file.copy)
```

```{r}
list.files("data/species", full.names = TRUE) %>% 
  walk2(., str_replace(., "data/", "output/"), file.copy)
```
